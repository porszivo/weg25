...
export class BooleanDeviceDetailsComponent implements OnInit {
    ...
    createChart(): void {
        ...
        let unitId = this.device.control_units.indexOf(this.controlUnit);

        let num = Number(sessionStorage.getItem(id + ':' + unitId));
        if (num > 0) {

            for (let i = 0; i < num; i++) {
                let value = Number(sessionStorage.getItem(id + ':' + unitId + ':' + i + ':value'));
